...
f.write("molecule,basis set,mo_basis,pop_method, C0, H1, H2,H3,H4, ChargeC, ChargeH1, ChargeH2, ChargeH3, ChargeH4\n")
    ...
    for basis in BASIS:
        for mo_basis in MO:
            for pop_method in POP:
                ...
                for i in range(5):
                    f.write(',')
                    f.write(str(res[decodense.decomp.CompKeys.tot][i]))
                for i in range(5):
                    f.write(',')
                    f.write(str(res[decodense.decomp.CompKeys.charge_atom][i]))
                f.write("\n")
