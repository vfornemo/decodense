...
def atoms(mol: gto.Mole, res: Dict[str, Any], unit: str) -> pd.DataFrame:
        ...
        if scalar_prop:
            prop = {comp_key: res[comp_key] * scaling for comp_key in res.keys() if comp_key != CompKeys.charge_atom}
            prop[CompKeys.tot] = prop[CompKeys.el] + prop[CompKeys.struct]
        else:
            prop = {comp_key + axis: res[comp_key][:, ax_idx] * scaling \
                    for comp_key in res.keys() for ax_idx, axis in enumerate((' (x)', ' (y)', ' (z)')) if comp_key != CompKeys.charge_atom}
